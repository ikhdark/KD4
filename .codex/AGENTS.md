...
state for this fork. Treat it as local workflow infrastructure for the active
checkout, not as upstream product source unless the user explicitly asks for
...
Hook commands must resolve files from the active checkout without embedding a
workstation-specific checkout path. Hook stdout is a wire contract: emit exactly
`{}` or the exact `SessionStart` injection object, with no newline or decoration.
Send every diagnostic to stderr and keep every handled failure fail-open.
