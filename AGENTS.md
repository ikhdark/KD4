...
* For source inventories, use `python scripts/source_inventory.py --query <query.json> --state <task-state.json>` with task-owned files outside the source tree. Query categories declare `name`, `paths` globs, optional `contains` regex, and `unresolved: true` for classifications still needing consumer inspection; supplied `candidates` are never automatically verified. Reuse the state file, inspect unresolved records selectively, and use `--paths` to produce the final tracked list without retyping filenames. Counts and paths are derived from the same validated records. Run bounded read-only inventories directly without a plan.
