...
Tracked repository path snapshot: `count=5329 sha256=d73271beec2a14f042f64715af706c8f48606e0e29ea59e9096ddecbdb8dd740`.
...
| Support crates and narrow binaries                | `codex-rs/async-utils`, `codex-rs/codex-experimental-api-macros`, `codex-rs/git-utils`, `codex-rs/stdio-to-uds`, `codex-rs/test-binary-support`, `codex-rs/uds`, `codex-rs/validation-contracts`                                                                                                                                                                                                                                                                                    |
...
<!-- BEGIN KD4 SOURCE OWNERS schema=2 manifest_sha256=fe24ccfb86d006806e5d4dadf423e53b48c8ab22b689431e609ed8e9bf972f4b -->
...
| `completion-proof-certification` | `scripts/completion_proof.py`<br>`scripts/completion_proof_canonical.py`<br>`scripts/completion_proof_inventory_v2.py`<br>`scripts/completion_proof_v2_live.py`<br>`codex-rs/core/src/completion_proof/v2_activation.rs`<br>`codex-rs/core/src/tools/handlers/inventory_activation.rs`<br>`scripts/materialize_completion_proof_inventory_v2.py`<br>`.codex/validation/child-validation-journal-v1.schema.json`<br>`codex-rs/validation-contracts/Cargo.toml`<br>`codex-rs/validation-contracts/src/applicability.rs`<br>`codex-rs/validation-contracts/src/canonical.rs`<br>`codex-rs/validation-contracts/src/inventory_v2.rs`<br>`codex-rs/validation-contracts/src/lib.rs`<br>`codex-rs/validation-contracts/src/path.rs`<br>`codex-rs/validation-contracts/src/receipts.rs`<br>`codex-rs/validation-contracts/src/recovery.rs`<br>`codex-rs/validation-contracts/src/runner.rs`<br>`codex-rs/validation-contracts/src/selection.rs` | `scripts/completion_proof.py::main`<br>`scripts/completion_proof_inventory_v2.py::validate_frozen_test_inventory_v2`<br>`scripts/materialize_completion_proof_inventory_v2.py::main`<br>`codex-rs/validation-contracts/src/inventory_v2.rs::FrozenTestInventoryV2` | `control_flow:calls` -> `path:scripts/completion_proof.py`<br>`control_flow:calls` -> `path:scripts/completion_proof_canonical.py`<br>`callers_consumers:consumed_by` -> `path:codex-rs/core/src/completion_proof.rs`<br>+9 more | `semantic:canonical-command-gate-separation`<br>`compatibility:immutable-baseline-reconciliation`<br>`compatibility:cross-language-validation-contract-parity` | `completion-proof-source-owner-focused`<br>`completion-proof-runner-focused`<br>`inventory-v2-python-focused`<br>`inventory-v2-rust-focused` |
| `completion-proof-focused-contracts` | `scripts/focused_live_successor_catalog.py`<br>`scripts/current_evidence_successor_projection.py`<br>`scripts/focused_replacement_approval_receipt.py`<br>`codex-rs/validation-contracts/src/focused_evidence_frame.rs`<br>`codex-rs/validation-contracts/src/focused_live_successor.rs`<br>`codex-rs/validation-contracts/src/focused_replacement_approval.rs`<br>`codex-rs/validation-contracts/src/historical_replacement_acceptance.rs` | `scripts/focused_live_successor_catalog.py::validate_focused_live_successor_catalog_semantics_v1`<br>`scripts/current_evidence_successor_projection.py::build_current_successor_projection_v1`<br>`scripts/focused_replacement_approval_receipt.py::validate_against_trusted_current_context_v1`<br>`codex-rs/validation-contracts/src/focused_evidence_frame.rs::FocusedEvidenceFrameV1`<br>`codex-rs/validation-contracts/src/focused_live_successor.rs::FocusedLiveSuccessorCatalogV1`<br>`codex-rs/validation-contracts/src/focused_live_successor.rs::parse_inventory_discovery_process_set_canonical_v1`<br>`codex-rs/validation-contracts/src/focused_replacement_approval.rs::FocusedReplacementApprovalReceiptV1`<br>`codex-rs/validation-contracts/src/historical_replacement_acceptance.rs::HistoricalReplacementAcceptanceProposalV1` | `callers_consumers:consumed_by` -> `path:scripts/completion_proof.py`<br>`callers_consumers:consumed_by` -> `path:codex-rs/core/src/completion_proof.rs`<br>`control_flow:calls` -> `path:codex-rs/agent-task-store/src/local.rs`<br>+4 more | `compatibility:historical-replacement-review-authority`<br>`compatibility:focused-evidence-frame-v1-binary-contract`<br>`compatibility:focused-evidence-ack-v1-binary-contract`<br>+1 more | `completion-proof-typed-canonical-focused`<br>`focused-live-successor-python-focused`<br>`completion-proof-current-evidence-focused`<br>`focused-live-successor-rust-focused`<br>`focused-evidence-frame-rust-focused`<br>`focused-replacement-approval-python-focused`<br>`focused-replacement-approval-rust-focused` |
| `configuration-generated-contracts` | `scripts/config_schema_check.py`<br>`codex-rs/core/src/bin/config_schema.rs`<br>`codex-rs/config/src/schema.rs`<br>`codex-rs/config/scripts/generate-proto.ps1`<br>`codex-rs/config/examples/generate-proto.rs`<br>`codex-rs/config/src/thread_config/proto/codex.thread_config.v1.proto` | `scripts/config_schema_check.py::main`<br>`codex-rs/core/src/bin/config_schema.rs::main`<br>`codex-rs/config/examples/generate-proto.rs::main` | `control_flow:calls` -> `path:codex-rs/core/src/bin/config_schema.rs`<br>`control_flow:calls` -> `path:codex-rs/config/examples/generate-proto.rs`<br>`callers_consumers:consumed_by` -> `path:codex-rs/core/src/config/schema_tests.rs`<br>+7 more | `compatibility:generated-configuration-source-parity` | `config-schema-generated-focused`<br>`config-proto-generated-focused` |
| `core-agent-runtime` | `codex-rs/core/src`<br>`codex-rs/core/tests`<br>`codex-rs/core/benches` | `codex-rs/core/src/session/mod.rs::Codex` | `callers_consumers:consumed_by` -> `owner:app-server-runtime`<br>`configuration:gated_by` -> `owner:feature-registry`<br>`tests_contracts:validated_by` -> `path:codex-rs/core/tests`<br>+1 more | `semantic:registered-tool-routing`<br>`semantic:ordinary-focused-test-quality` | `core-focused-test-quality`<br>`core-focused` |
| `exec-server-relay-contracts` | `codex-rs/exec-server/examples/generate-relay-proto.rs`<br>`codex-rs/exec-server/src/proto/codex.exec_server.relay.v1.proto` | `codex-rs/exec-server/examples/generate-relay-proto.rs::main` | `control_flow:calls` -> `path:codex-rs/exec-server/src/proto/codex.exec_server.relay.v1.proto`<br>`callers_consumers:consumed_by` -> `path:codex-rs/exec-server/src/relay_proto.rs`<br>`control_flow:consumed_by` -> `path:codex-rs/exec-server/src/relay.rs`<br>+3 more | `compatibility:relay-proto-source-parity` | `exec-server-relay-proto-generated-focused` |
...
| `replacement-admission-certification` | `scripts/replacement_admission.py`<br>`.codex/validation/replacement-admissions-v1.schema.json`<br>`.codex/validation/replacement-admissions-v1.json` | `scripts/replacement_admission.py::main` | `control_flow:calls` -> `path:scripts/replacement_admission.py`<br>`configuration:reads_config` -> `config:.codex/validation/replacement-admissions-v1.json`<br>`configuration:reads_config` -> `config:.codex/validation/test-replacements-v2.json`<br>+1 more | `semantic:replacement-admission-dormant-projection`<br>`compatibility:replacement-successor-catalog-boundary`<br>`semantic:replacement-trusted-defect-receipt-provenance-boundary` | `replacement-admission-cli-focused`<br>`replacement-admission-source-owner-focused` |
...
| `repository-maintenance-runners` | `scripts/just-shell.py`<br>`scripts/readme_toc.py`<br>`scripts/root_maintenance.py`<br>`scripts/source_map_check.py`<br>`scripts/rust_test_runner.py` | `scripts/just-shell.py::main`<br>`scripts/readme_toc.py::main`<br>`scripts/root_maintenance.py::main`<br>`scripts/source_map_check.py::main`<br>`scripts/rust_test_runner.py::main` | `control_flow:calls` -> `path:scripts/just-shell.py`<br>`control_flow:calls` -> `owner:source-owner-index`<br>`control_flow:calls` -> `path:scripts/readme_toc.py`<br>+10 more | `semantic:maintenance-runner-route-fidelity` | `maintenance-owner-closure-focused`<br>`readme-toc-owner-closure-focused`<br>`completion-proof-maintenance-routing-focused` |
| `rust-turn-latency-benchmark` | `codex-rs/core/benches/turn_latency.rs`<br>`codex-rs/core/benches/turn_latency/ab_contract.rs`<br>`codex-rs/core/benches/turn_latency/ab_runner.rs`<br>`codex-rs/core/benches/turn_latency/runtime_fixtures.rs`<br>`codex-rs/core/benches/turn_latency/tests.rs`<br>`codex-rs/core/tests/turn_latency_bench.rs`<br>`docs/benchmarks/turn-latency/README.md`<br>`scripts/kd4_live_agent_benchmark.py`<br>`scripts/test_kd4_live_agent_benchmark.py` | `codex-rs/core/benches/turn_latency.rs::main`<br>`scripts/kd4_live_agent_benchmark.py::main` | `generated_artifacts:emits` -> `path:docs/benchmarks/turn-latency/accepted`<br>`control_flow:calls` -> `owner:core-agent-runtime`<br>`callers_consumers:consumed_by` -> `path:codex-rs/core/Cargo.toml`<br>+3 more | `semantic:separate-capture-and-comparison-verdicts`<br>`semantic:accepted-turn-latency-reports-are-verified`<br>`semantic:live-agent-source-and-scoring-integrity` | `rust-turn-latency-focused`<br>`live-agent-benchmark-focused` |
...
| `workflow-preflight-coordination` | `scripts/workflow_preflight.py` | `scripts/workflow_preflight.py::main` | `control_flow:calls` -> `path:scripts/workflow_preflight.py`<br>`callers_consumers:consumed_by` -> `path:justfile`<br>`callers_consumers:consumed_by` -> `path:.codex/harness/workflow.md`<br>+5 more | `compatibility:workflow-preflight-contract-fidelity` | `workflow-preflight-focused`<br>`workflow-preflight-owner-closure-focused` |
