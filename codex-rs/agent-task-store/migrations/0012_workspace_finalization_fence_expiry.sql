-- Keep migration 0011 immutable for databases that already recorded its checksum.
-- Recreate every finalization fence trigger and exclusive index so an elapsed
-- active or dispatching row cannot strand mutations after its owner exits.
...
DROP INDEX IF EXISTS workspace_finalization_fences_active_workspace_idx;
...
WHEN EXISTS (
    ...
    WHERE fence.workspace_id = NEW.workspace_id AND fence.state IN ('active', 'dispatching')
      AND julianday(json_extract(fence.expires_at, '$')) > julianday('now')
)
BEGIN
...
WHEN EXISTS (
    ...
    WHERE repository.assignment_id = NEW.assignment_id AND fence.state IN ('active', 'dispatching')
      AND julianday(json_extract(fence.expires_at, '$')) > julianday('now')
)
BEGIN
    SELECT RAISE(ABORT, 'workspace finalization active');
...
WHEN EXISTS (
    ...
    JOIN workspace_finalization_fences AS fence
      ON fence.workspace_id = repository.workspace_id
    WHERE repository.assignment_id = OLD.assignment_id AND fence.state IN ('active', 'dispatching')
      AND julianday(json_extract(fence.expires_at, '$')) > julianday('now')
)
BEGIN
    SELECT RAISE(ABORT, 'workspace finalization active');
...
WHEN EXISTS (
    ...
    JOIN workspace_finalization_fences AS fence
      ON fence.workspace_id = repository.workspace_id
    WHERE attempt.attempt_id = NEW.attempt_id AND fence.state IN ('active', 'dispatching')
      AND julianday(json_extract(fence.expires_at, '$')) > julianday('now')
)
BEGIN
    SELECT RAISE(ABORT, 'workspace finalization active');
...
WHEN EXISTS (
    ...
    JOIN workspace_finalization_fences AS fence
      ON fence.workspace_id = repository.workspace_id
    WHERE attempt.attempt_id = OLD.attempt_id AND fence.state IN ('active', 'dispatching')
      AND julianday(json_extract(fence.expires_at, '$')) > julianday('now')
)
BEGIN
    SELECT RAISE(ABORT, 'workspace finalization active');
...
WHEN EXISTS (
    SELECT 1 FROM workspace_finalization_fences AS fence
    WHERE fence.workspace_id = OLD.workspace_id AND fence.state IN ('active', 'dispatching')
      AND julianday(json_extract(fence.expires_at, '$')) > julianday('now')
)
BEGIN
    SELECT RAISE(ABORT, 'workspace finalization active');
