...
#[tokio::test]
async fn review_target_receipt_sealing_rejects_amended_target_and_accepts_current_success() {
    let fixture = Fixture::new().await;
    let (target, initial_target_attempt) = fixture
        .store
        .create_assignment(
            fixture.repo.path(),
            worker_draft("review-target-seal-root", "src"),
        )
        .await
    ...
    fixture
        ...
        .submit_agent_receipt_with_review(
            initial_target_attempt.attempt_id,
            completed_receipt(Vec::new()),
            "cold review required: exercise atomic target sealing".to_string(),
        )
        .await
        .expect("initial target receipt");
    let (reviewer, reviewer_attempt) = fixture
        .store
        .create_assignment(
            fixture.repo.path(),
            relation_draft(
                "review-target-seal-root",
                AgentRole::Reviewer,
    ...
    let capsule = serde_json::to_string(&audit_capsule(&reviewer, &reviewer_attempt))
        .expect("reviewer capsule serializes");
    let reviewer = fixture
        .store
        .attach_task_capsule(reviewer.assignment_id, reviewer_attempt.attempt_id, capsule)
        .await
        .expect("fresh reviewer capsule attaches");
    assert!(reviewer.task_capsule.is_some());
    fixture
        .store
        .set_agent_gate(
            ...
            GateKind::Review,
            GateStatus::ChangesRequested,
            "the target needs one correction".to_string(),
        )
        .await
        .expect("review requests correction");
    let current_target_attempt = fixture
        .store
        .amend_agent_task(
            TaskActor::Root,
            target.assignment_id,
        ...
        .await
        .expect("target correction attempt");
    fixture
        .store
        .submit_agent_receipt(
            current_target_attempt.attempt_id,
            completed_receipt(Vec::new()),
        )
        .await
        .expect("current target receipt");
    ...
    assert!(matches!(
        fixture
            ...
            .await,
        Err(StoreError::AttemptNotActive(attempt_id))
            if attempt_id == initial_target_attempt.attempt_id
    ));
    let unsealed_reviewer = fixture
        .store
        .get_agent_task(reviewer.assignment_id, Some(0))
        .await
        .expect("reviewer task after stale target rejection");
    assert_eq!(
        unsealed_reviewer.current_attempt.state,
        AttemptState::Active
    );
    assert!(unsealed_reviewer.receipt.is_none());

    let mut mismatched_reviewer = reviewer.clone();
    mismatched_reviewer.objective = "caller-authored reviewer identity".to_string();
    assert!(matches!(
        fixture
            .store
            .submit_agent_receipt_for_review_target(
            ...
            .await,
        Err(StoreError::InvalidAssignment(message))
            if message.contains("fresh-spawn snapshot")
    ));

    let reviewer_receipt = fixture
        .store
        .submit_agent_receipt_for_review_target(
            reviewer_attempt.attempt_id,
            completed_receipt(Vec::new()),
        ...
        .await
        .expect("current successful target authorizes reviewer sealing");
    assert_eq!(reviewer_receipt.status, AgentStatusClaim::Completed);
}
