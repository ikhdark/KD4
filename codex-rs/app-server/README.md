...
- Saturation at any bounded request or event queue is reported with JSON-RPC error code `-32001`.
- The error data contains a typed `reason` identifying the saturated queue and `retryable: true`; clients should use that data to apply exponential backoff with jitter.
...
- `thread/items/list` — experimental; page through persisted thread items without resuming the thread. Pass `turnId` to restrict results to one turn, or omit it to page items across the thread. Stores with a projected item index provide native pagination; legacy/local stores fall back to reconstructing items from persisted rollout history.
...
- `model/list` — list available models (set `includeHidden: true` to include entries with `hidden: true`), with model-advertised string reasoning effort options in the catalog's intended progression order, `serviceTiers`, optional `defaultServiceTier`, optional legacy `upgrade` model ids, optional `upgradeInfo` metadata (`model`, `upgradeCopy`, `modelLink`, `migrationMarkdown`), and optional `availabilityNux` metadata. Deprecated `additionalSpeedTiers` values may still appear for compatibility; new clients should use `serviceTiers` and canonical tier ids such as `priority`. Clients should preserve the `supportedReasoningEfforts` array order rather than deriving order from the effort names.
...
- `item/tool/requestUserInput` — prompt the user with 1–3 short questions for a tool call and return their answers (experimental).
...
{ "method": "thread/start", "id": 10, "params": {
    ...
    // "runtimeWorkspaceRoots": ["C:\\work\\project", "C:\\work\\openai"],
...
{ "method": "thread/list", "id": 20, "params": {
    "cursor": null,
    "limit": 25,
    "cwd": ["C:\\work\\project", "C:\\work\\project-worktree"],
    "sortKey": "created_at"
} }
{ "id": 20, "result": {
    "data": [
...
Omit `turnId` or pass `null` to page items across the thread. If the active store does not implement item pagination but can return persisted rollout history, app-server reconstructs the items locally and returns the same cursor-based response. Stores that provide neither item pagination nor readable history return JSON-RPC `-32601`.
...
{ "method": "turn/start", "id": 30, "params": {
    ...
    "environments": [
        { "environmentId": "local", "cwd": "C:\\work\\project" }
    ],
    "approvalPolicy": "unlessTrusted",
    "sandboxPolicy": {
        "type": "workspaceWrite",
        "writableRoots": ["C:\\work\\project"],
        "networkAccess": true
    },
    // Prefer experimental profile selection:
    // "permissions": ":workspace"
...
{ "method": "turn/start", "id": 33, "params": {
    ...
    "input": [
        ...
        { "type": "skill", "name": "skill-creator", "path": "C:\\Users\\me\\.codex\\skills\\skill-creator\\SKILL.md" }
    ]
} }
{ "id": 33, "result": { "turn": {
    "id": "turn_457",
...
Use `thread/injectItems` to append prebuilt Responses API items to a loaded thread’s prompt history without starting a user turn. These items are persisted to the rollout and included in subsequent model requests. Any `input_image` items must use inline data URLs; remote HTTP(S) image URLs are rejected.

```json
{ "method": "thread/injectItems", "id": 36, "params": {
    "threadId": "thr_123",
    "items": [
        {
            "type": "message",
...
{ "method": "command/exec", "id": 32, "params": {
    ...
    "processId": "dir-1",                          // optional string; required for streaming and ability to terminate the process
    "cwd": "C:\\work\\project",                  // optional; defaults to server cwd
...
{ "method": "process/spawn", "id": 40, "params": {
    ...
    "cwd": "C:\\work\\project",                  // required absolute path
...
{ "method": "fs/changed", "params": {
    "watchId": "0195ec6b-1d6f-7c2e-8c7a-56f2c4a8b9d1",
    "changedPaths": ["C:\\work\\project\\.git\\HEAD"]
} }
...
```json
{
  ...
  "params": {
    ...
    "permissions": {
      "fileSystem": {
        "write": ["C:\\work\\project", "C:\\work\\shared"]
      }
    }
  }
}
...
3. Client response.
4. `item/completed` with `item.type = "dynamicToolCall"`, final `status`, the returned `contentItems`/`success`, and an `error` message when the call was cancelled before a response arrived.
...
{ "method": "skills/list", "id": 25, "params": {
    "cwds": ["C:\\work\\project", "C:\\work\\other-project"],
    "forceReload": true
} }
{ "id": 25, "result": {
    "data": [{
...
```json
{
  ...
  "id": 26,
  "params": {
    "extraRoots": ["C:\\work\\generated-skills"]
  }
}
{ "id": 26, "result": {} }
```
...
```json
{
  ...
  "id": 27,
  "params": {
    "path": "C:\\Users\\alice\\.codex\\skills\\skill-creator\\SKILL.md",
    "name": null,
    "enabled": false
  }
}
...
```json
{
  ...
  "result": {
    "data": [{
      "cwd": "C:\\work\\project",
      "hooks": [{
        "key": "C:\\Users\\me\\.codex\\config.toml:pre_tool_use:0:0",
        "eventName": "pre_tool_use",
        "handlerType": "command",
        "isManaged": false,
        "matcher": "Bash",
        "command": "python.exe C:\\Users\\me\\hook.py",
        "timeoutSec": 5,
        "statusMessage": "running hook",
        "sourcePath": "C:\\Users\\me\\.codex\\config.toml",
        "source": "user",
        "pluginId": null,
        "displayOrder": 0,
        "enabled": true,
...
```json
{
  ...
  "params": {
    "edits": [{
      "keyPath": "hooks.state",
      "value": {
        "C:\\Users\\me\\.codex\\config.toml:pre_tool_use:0:0": {
          "enabled": false
        }
      },
...
4. Regenerate protocol fixtures:
   ...
   just app-server-schema-regenerate <owner> --experimental
   ```
