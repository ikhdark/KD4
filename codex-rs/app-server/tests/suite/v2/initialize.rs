...
#[test]
fn capture_helper_preserves_payloads_paths_and_rejects_extra_arguments() -> Result<()> {
    let capture = cargo_bin("codex-app-server-test-notify-capture")?;
    let dir = TempDir::new()?;
...
#[test]
fn capture_helper_preserves_native_paths_and_rejects_invalid_payload_encoding() -> Result<()> {
    #[cfg(unix)]
    let invalid = {
