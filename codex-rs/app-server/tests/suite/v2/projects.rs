...
async fn projects_list_by_recency_and_preserve_metadata_timestamps() -> Result<()> {
    ...
    write_project_test_config(codex_home.path(), &responses.uri())?;
...
async fn projects_persist_and_assign_threads() -> Result<()> {
    ...
    let started_id = server
        ...
        .await?;
    let JSONRPCMessage::Response(response) = read_project_message(&mut server).await? else {
        panic!("thread/start must respond before thread/started");
    };
    assert_eq!(response.id, RequestId::Integer(started_id));
    let started: ThreadStartResponse = serde_json::from_value(response.result)?;
    let JSONRPCMessage::Notification(thread_started) = read_project_message(&mut server).await?
    else {
    ...
    let ephemeral_id = server
        ...
        .await?;
    let JSONRPCMessage::Response(response) = read_project_message(&mut server).await? else {
        panic!("ephemeral thread/start must respond before thread/started");
    };
...
async fn project_import_is_atomic_and_notifies_after_commit_in_order() -> Result<()> {
    ...
    let JSONRPCMessage::Notification(project_changed) = read_project_message(&mut server).await?
    else {
    ...
    let JSONRPCMessage::Notification(thread_updated) = read_project_message(&mut server).await?
    else {
...
async fn projects_validate_filters_cursors_and_sqlite_less_assignment() -> Result<()> {
    ...
    let mut unsupported_projects = TestAppServer::builder()
        ...
        .build()
        .await?;
    unsupported_projects.initialize().await?;
...
async fn assigned_forks_inherit_projects_for_persistent_and_ephemeral_children() -> Result<()> {
    ...
    let fork_id = server
        ...
        .await?;
    let JSONRPCMessage::Response(response) = read_project_message(&mut server).await? else {
        panic!("thread/fork must respond before thread/started");
    };
    ...
    let ephemeral_id = server
        ...
        .await?;
    let JSONRPCMessage::Response(response) = read_project_message(&mut server).await? else {
        panic!("ephemeral thread/fork must respond before thread/started");
    };
    assert_eq!(response.id, RequestId::Integer(ephemeral_id));
...
// Adapt the upstream feature scenarios to this fork's existing test transport.
trait ProjectTestClient {
    async fn request<T: serde::de::DeserializeOwned>(
        &mut self,
        build: impl FnOnce(RequestId) -> ClientRequest,
    ) -> Result<T>;
    async fn start_thread(&mut self, params: ThreadStartParams) -> Result<ThreadStartResponse>;
    async fn read_notification<T: serde::de::DeserializeOwned>(
        &mut self,
        method: &str,
    ) -> Result<T>;
}
impl ProjectTestClient for TestAppServer {
    async fn request<T: serde::de::DeserializeOwned>(
        &mut self,
        build: impl FnOnce(RequestId) -> ClientRequest,
    ) -> Result<T> {
        let value = serde_json::to_value(build(RequestId::Integer(0)))?;
        let method = value["method"]
            .as_str()
            .ok_or_else(|| anyhow::anyhow!("request method missing"))?;
        let id = self
            .send_raw_request(method, value.get("params").cloned())
            .await?;
        app_test_support::to_response(
            self.read_stream_until_response_message(RequestId::Integer(id))
                .await?,
        )
    }
    async fn start_thread(&mut self, params: ThreadStartParams) -> Result<ThreadStartResponse> {
        let id = self.send_thread_start_request_with_auto_env(params).await?;
        app_test_support::to_response(
            self.read_stream_until_response_message(RequestId::Integer(id))
                .await?,
        )
    ...
    async fn read_notification<T: serde::de::DeserializeOwned>(
        ...
        Ok(serde_json::from_value(notification.params.ok_or_else(
            || anyhow::anyhow!("notification params missing"),
        )?)?)
    }
}

fn write_project_test_config(home: &std::path::Path, server_uri: &str) -> std::io::Result<()> {
    // SQLite is always enabled in this fork; use its existing mock provider fixture.
...
async fn read_project_message(server: &mut TestAppServer) -> Result<JSONRPCMessage> {
    loop {
        let message = server.read_next_message().await?;
        // These asynchronous messages can arrive after a previous start/turn response.
