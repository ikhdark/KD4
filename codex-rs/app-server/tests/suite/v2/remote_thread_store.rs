...
//! config-driven selection path without touching local rollout storage or duplicating
//! remote threads into the local state database.
//!
...
//! the temporary `codex_home` must not contain rollout session files or local thread
//! rows. Orthogonal workspace coordination may legitimately initialize SQLite.
...
async fn thread_delete_with_non_local_thread_store_does_not_create_local_persistence() -> Result<()>
{
    ...
    let started_thread_id = ThreadId::from_string(&thread.id)?;
    ...
    assert_no_local_thread_persistence(codex_home.path(), &[started_thread_id, unloaded_thread_id])
        .await?;
...
async fn assert_no_local_thread_persistence(
    codex_home: &Path,
    thread_ids: &[ThreadId],
) -> Result<()> {
    assert!(
    ...
    let state =
        codex_state::StateRuntime::init(codex_home.to_path_buf(), "mock_provider".to_string())
            .await?;
    for thread_id in thread_ids {
        assert_eq!(
            state.get_thread(*thread_id).await?,
            None,
            "non-local thread persistence should not create a local row for {thread_id}"
        );
    }
