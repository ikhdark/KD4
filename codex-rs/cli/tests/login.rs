...
fn codex_command(codex_home: &Path) -> Result<assert_cmd::Command> {
    ...
    cmd.env("CODEX_HOME", codex_home).env_remove("TERM");
    Ok(cmd)
}
...
async fn device_login_replaces_existing_auth_only_after_success() -> Result<()> {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn failed_device_login_preserves_existing_auth() -> Result<()> {
    let server = MockServer::start().await;
    Mock::given(method("POST"))
        .and(path("/api/accounts/deviceauth/usercode"))
    ...
    let codex_home = TempDir::new()?;
    write_file_auth_config(codex_home.path())?;
    let original = json!({"OPENAI_API_KEY": "sk-existing"});
    std::fs::write(
        codex_home.path().join("auth.json"),
    ...
    codex_command(codex_home.path())?
        ...
        .assert()
        .failure()
        .stderr(contains("Error logging in"));
