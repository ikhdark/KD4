...
#[test]
fn for_prompt_retires_only_old_successful_outputs_without_mutating_history() {
    ...
    for index in 0..22 {
        ...
        items.push(ResponseItem::FunctionCallOutput {
            id: Some(format!("fco_{index}")),
            call_id,
            output: FunctionCallOutputPayload {
                body: FunctionCallOutputBody::Text(format!("successful output {index}")),
                success: Some(true),
            },
            internal_chat_message_metadata_passthrough: None,
        });
    ...
    assert_eq!(prompt.len(), 44, "call/output pairs must remain intact");
    for index in 0..22 {
        let call_id = format!("call-{index}");
        assert!(matches!(
            (&prompt[index * 2], &prompt[index * 2 + 1]),
            (
                ResponseItem::FunctionCall {
                    call_id: call,
                    ..
    ...
    for index in 0..2 {
        let ResponseItem::FunctionCallOutput { output, .. } = &prompt[index * 2 + 1] else {
            panic!("expected function output");
        };
        assert_eq!(output.text_content(), Some(PROMPT_RETIRED_SUCCESS_OUTPUT));
    }
    let ResponseItem::FunctionCallOutput { output, .. } = &prompt[2 * 2 + 1] else {
        panic!("expected retained function output");
    };
    assert_eq!(output.text_content(), Some("successful output 2"));

    let ResponseItem::FunctionCallOutput { output, .. } = &history.raw_items()[1] else {
        panic!("expected original function output");
    };
    assert_eq!(
        output.text_content(),
        Some("successful output 0"),
        "prompt retirement must not alter persisted/live history"
    );
}

#[test]
fn for_prompt_preserves_old_failed_and_unknown_outputs() {
    let outputs = [("failed", Some(false)), ("unknown", None)];
    let mut items = Vec::new();
    for (index, (text, success)) in outputs.into_iter().enumerate() {
        let call_id = format!("special-{index}");
        items.push(ResponseItem::FunctionCall {
            id: None,
            name: "diagnostic".to_string(),
            namespace: None,
    ...
    for index in 0..22 {
        let call_id = format!("success-{index}");
        items.push(ResponseItem::FunctionCall {
            id: None,
            name: "diagnostic".to_string(),
            namespace: None,
        ...
        items.push(ResponseItem::FunctionCallOutput {
            ...
            call_id,
            output: FunctionCallOutputPayload {
                body: FunctionCallOutputBody::Text(format!("success {index}")),
                success: Some(true),
            },
            internal_chat_message_metadata_passthrough: None,
        });
    ...
    assert_eq!(preserved, vec!["failed", "unknown"]);
}

#[test]
fn for_prompt_retires_completed_tool_search_outputs_only() {
    let mut items = Vec::new();
    for index in 0..22 {
        ...
        items.push(ResponseItem::ToolSearchOutput {
            id: None,
    ...
    items.push(ResponseItem::ToolSearchCall {
        id: None,
        call_id: Some("failed-search".to_string()),
    ...
    items.push(ResponseItem::ToolSearchOutput {
        ...
        execution: "client".to_string(),
        tools: vec![serde_json::json!({"error": "preserve me"})],
        internal_chat_message_metadata_passthrough: None,
    });

    let history = create_history_with_items(items);
    let prompt = history.clone().for_prompt(&default_input_modalities());
    let tool_search_outputs = prompt
        .iter()
        .filter_map(|item| match item {
            ResponseItem::ToolSearchOutput { status, tools, .. } => Some((status, tools)),
            _ => None,
        })
        .collect::<Vec<_>>();

    assert!(tool_search_outputs[0].1.is_empty());
    assert!(tool_search_outputs[1].1.is_empty());
    let (status, tools) = tool_search_outputs
        .last()
        .expect("failed output should remain");
    assert_eq!(status.as_str(), "failed");
    assert_eq!(
        tools.as_slice(),
        &[serde_json::json!({"error": "preserve me"})]
    );
    let ResponseItem::ToolSearchOutput { tools, .. } = &history.raw_items()[1] else {
        panic!("expected original tool-search output");
    };
    assert!(
        !tools.is_empty(),
        "prompt retirement must not mutate history"
    );
}

#[test]
fn prompt_estimate_includes_tools_actual_instructions_and_output_schema() {
    let prompt = Prompt {
        input: Vec::new(),
        tools: vec![ToolSpec::Freeform(FreeformTool {
            name: "large".to_string(),
            description: "x".repeat(640_000),
            format: FreeformToolFormat {
                r#type: "grammar".to_string(),
                syntax: "lark".to_string(),
                definition: "start: \"large\"".to_string(),
            },
        })],
        parallel_tool_calls: false,
        base_instructions: BaseInstructions {
            text: "session-specific instructions".to_string(),
        },
    ...
    let estimate = ContextManager::estimate_prompt_token_count(&prompt);

    assert!(
        estimate.is_some_and(|estimate| {
            estimate >= crate::session::context_window::LOCAL_CONTEXT_WINDOW_LIMIT
        }),
        "tool and output-schema bytes must participate in the 272K preflight"
    );
}

#[test]
fn for_prompt_caps_retained_success_output_tokens_but_keeps_the_newest_four() {
    let mut items = Vec::new();
    for index in 0..5 {
        let call_id = format!("large-{index}");
        items.push(ResponseItem::FunctionCall {
            id: None,
            name: "diagnostic".to_string(),
            namespace: None,
    ...
    assert_eq!(outputs[0], PROMPT_RETIRED_SUCCESS_OUTPUT);
    for (index, output) in outputs.iter().enumerate().skip(1) {
        assert!(
            output.starts_with(&format!("large output {index}:")),
            "the four newest successful outputs must survive prompt retirement"
        );
    }
}
