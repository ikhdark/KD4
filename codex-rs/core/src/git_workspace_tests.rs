...
fn ordinary_workspace_identity_does_not_request_patch_or_content_materialization() {
    let status_args = workspace_generation_status_args();
    let index_args = workspace_generation_index_args();

    assert!(status_args.contains(&"status"));
    assert!(status_args.contains(&"--porcelain=v2"));
    assert_eq!(
        index_args,
        [
    ...
    for expensive_arg in ["diff", "--patch", "--binary", "hash-object"] {
        assert!(!status_args.contains(&expensive_arg));
        assert!(!index_args.contains(&expensive_arg));
    }
}

#[tokio::test]
async fn workspace_identity_rejects_skip_worktree_hidden_mutation_from_real_git() {
    ...
    .await;
    std::fs::write(repo.join("README.md"), "hidden mutation\n").expect("write hidden mutation");
    ...
    assert!(
        ...
        "mutation should be hidden from status"
    );
    assert_eq!(
...
#[tokio::test]
async fn workspace_identity_rejects_assume_unchanged_hidden_mutation_from_real_git() {
    let (_temp, repo) = create_clean_git_repo().await;
    run_git(
        repo.as_path(),
...
#[tokio::test]
async fn workspace_identity_rejects_true_sparse_index_from_real_git() {
    let (_temp, repo) = create_clean_git_repo().await;
    std::fs::create_dir_all(repo.join(".codex")).expect("create included directory");
    std::fs::create_dir_all(repo.join("excluded")).expect("create excluded directory");
    std::fs::write(repo.join(".codex/visible.txt"), "visible\n").expect("write included file");
    std::fs::write(
        repo.join("excluded/rogue_test.py"),
        "def test_rogue(): pass\n",
    )
    .expect("write excluded test");
    run_git(repo.as_path(), &["add", ".codex", "excluded"]).await;
    run_git(repo.as_path(), &["commit", "-q", "-m", "sparse inputs"]).await;
    run_git(
        repo.as_path(),
        &["sparse-checkout", "init", "--cone", "--sparse-index"],
    )
    .await;
    run_git(repo.as_path(), &["sparse-checkout", "set", ".codex"]).await;

    let sparse_storage = run_git(
        repo.as_path(),
        &[
            "ls-files", "--cached", "--stage", "-v", "-f", "-z", "--sparse",
        ],
    )
    .await;
    assert!(
        sparse_storage
            .stdout
            .windows(b"S 040000".len())
            .any(|window| window == b"S 040000"),
        "Git did not retain a real sparse-directory index entry: {:?}",
        String::from_utf8_lossy(&sparse_storage.stdout)
    );
    let exact_index = run_git(repo.as_path(), workspace_generation_index_args()).await;
    assert!(
        exact_index
            .stdout
            .split(|byte| *byte == 0)
            .any(|record| record.starts_with(b"S ")),
        "the exact proof command did not expose the hidden sparse entry: {:?}",
        String::from_utf8_lossy(&exact_index.stdout)
    );
    assert_eq!(
        capture_workspace_evidence_identity(repo.as_path()).await,
        None
...
#[tokio::test]
async fn workspace_identity_rejects_gitlink_entry_from_real_git() {
    let (_temp, repo) = create_clean_git_repo().await;
    let head = String::from_utf8(run_git(repo.as_path(), &["rev-parse", "HEAD"]).await.stdout)
        .expect("HEAD is UTF-8")
        .trim()
        .to_string();
    let cache_info = format!("160000,{head},vendor");
    run_git(
        repo.as_path(),
...
#[tokio::test]
async fn workspace_identity_rejects_absent_test_symlink_from_real_git() {
    let (_temp, repo) = create_clean_git_repo().await;
    let target = repo.join("link-target");
    std::fs::write(&target, "elsewhere\n").expect("write link target blob");
    let object_id = String::from_utf8(
        run_git(repo.as_path(), &["hash-object", "-w", "link-target"])
            .await
            .stdout,
    )
    ...
    .trim()
    .to_string();
    std::fs::remove_file(target).expect("remove link target fixture");
    let cache_info = format!("120000,{object_id},missing_test.py");
    run_git(
        repo.as_path(),
        &["update-index", "--add", "--cacheinfo", &cache_info],
    )
    ...
    assert!(!repo.join("missing_test.py").exists());
    assert_eq!(
        capture_workspace_evidence_identity(repo.as_path()).await,
        None
    );
...
#[tokio::test]
async fn workspace_identity_preserves_non_test_symlink_and_missing_regular_files() {
    let (_temp, repo) = create_clean_git_repo().await;
    let target = repo.join("link-target");
    std::fs::write(&target, "elsewhere\n").expect("write link target blob");
    let object_id = String::from_utf8(
    ...
    .to_string();
    std::fs::remove_file(target).expect("remove link target fixture");
    let cache_info = format!("120000,{object_id},docs-current");
    run_git(
        repo.as_path(),
        &["update-index", "--add", "--cacheinfo", &cache_info],
    )
    .await;
    std::fs::remove_file(repo.join("README.md")).expect("remove tracked regular file");
...
#[test]
fn workspace_generation_index_parser_fails_closed_on_unsupported_entries() {
    let object_id = "a".repeat(40);
    assert_eq!(
        workspace_generation_index(format!("H 100644 {object_id} 0\tsrc/runtime.rs\0").as_bytes()),
        Some(())
    );
    assert_eq!(
        workspace_generation_index(format!("H 120000 {object_id} 0\tdocs/current\0").as_bytes()),
        Some(())
    );
    for invalid in [
        format!("S 040000 {object_id} 0\tsrc\0"),
        format!("h 100644 {object_id} 0\tsrc/runtime.rs\0"),
        format!("H 160000 {object_id} 0\tvendor\0"),
        format!("H 120000 {object_id} 0\tmissing_test.py\0"),
        format!("H 100644 {object_id} 1\tsrc/runtime.rs\0"),
        format!("H 100644 {object_id} 0\tsrc/runtime.rs"),
        format!("H 100644 {object_id} 0\tsrc/runtime.rs\0\0"),
        format!("H 100644 {object_id} 0\tsrc/runtime.rs\0H 100644 {object_id} 0\tsrc/runtime.rs\0"),
        format!("H 100644 {object_id} 0\tsrc/Runtime.rs\0H 100644 {object_id} 0\tsrc/runtime.rs\0"),
        format!("H 100644 {object_id} 0\t../runtime.rs\0"),
        format!("H 100644 {object_id} 0\tsrc\\runtime.rs\0"),
    ] {
        assert_eq!(
            workspace_generation_index(invalid.as_bytes()),
            None,
            "{invalid:?}"
        );
    }
...
#[test]
fn workspace_generation_status_parser_preserves_rename_endpoints_and_fails_closed() {
    let status = format!(
...
#[test]
fn source_journal_ignores_only_runtime_owned_barrier_events() {
    let root = TempDir::new().expect("source observation root");
    let repository_root = root.path().to_path_buf();
    let source = repository_root.join("src").join("lib.rs");
    let barrier = repository_root.join(".completion-proof-watcher-barrier-private");
    let lookalike = repository_root.join(".completion-proof-watcher-barrier-user");
    let observation = SourcePathChangeObservation {
        watcher_epoch: 1,
    ...
    let journal = StdMutex::new(SourceChangeJournal::default());

    let registration = RuntimeOwnedSourcePathRegistration::register(&journal, &barrier)
        .expect("register private barrier");
    journal.lock().expect("source journal").record(
        1,
        Some(vec![source.clone(), barrier.clone(), lookalike.clone()]),
    );
    drop(registration);
    ...
    let changed = journal
        ...
        .exact_paths_since(&observation)
        .expect("exact source evidence")
        .into_iter()
        .collect::<BTreeSet<_>>();
    assert_eq!(changed, BTreeSet::from([source, lookalike]));
    ...
    journal
        .lock()
        .expect("source journal")
        .record(2, Some(vec![barrier.clone()]));
    assert_eq!(
        journal
            .lock()
            .expect("source journal")
            .exact_paths_since(&continuation),
        Some(vec![barrier]),
        "the same path is ordinary evidence after runtime ownership ends"
    );
}

#[test]
fn source_journal_ignores_runtime_barrier_ancestor_only_in_the_same_event() {
    let root = TempDir::new().expect("source observation root");
    let repository_root = root.path().to_path_buf();
    let barrier = repository_root.join(".completion-proof-watcher-barrier-private");
    let observation = SourcePathChangeObservation {
    ...
    journal
        .lock()
        .expect("source journal")
        .record(1, Some(vec![repository_root.clone(), barrier.clone()]));
    assert!(
        !journal
            .lock()
            .expect("source journal")
            .path_changed_since(&observation),
        "an ancestor delivered with the private barrier is part of that runtime event"
    );
    drop(registration);
    ...
    journal
        .lock()
        .expect("source journal")
        .record(2, Some(vec![repository_root]));
    assert!(
        journal
            .lock()
            .expect("source journal")
            .path_changed_since(&continuation),
        "a standalone repository-root event must remain fail closed"
    );
}

#[cfg(windows)]
#[tokio::test]
async fn source_observation_handoff_barrier_only_stays_quiet() {
    let root = TempDir::new().expect("source observation root");
    let cache = GitWorkspaceCache::new();
    let observation = cache
        .begin_source_path_change_observation_after_barrier(root.path())
        .await
        .expect("begin source observation after barrier");

    let first = cache
        .finish_source_path_change_observation_and_continue(&observation)
        .await
        .expect("finish quiet source observation");
    assert!(
        !first.had_non_runtime_change,
        "the handoff barrier itself must not count as a workspace change"
    );
    assert!(
        first.exact_paths.is_empty(),
        "the handoff barrier itself must not produce exact source evidence"
    );

    let second = cache
        .finish_source_path_change_observation_and_continue(&first.continuation)
        .await
        .expect("finish continuation after quiet handoff");
    assert!(
        !second.had_non_runtime_change,
        "a late companion event from the prior barrier must not leak into its continuation"
    );
    assert!(
        second.exact_paths.is_empty(),
        "a late companion event from the prior barrier must not become exact source evidence"
    );
}

#[cfg(windows)]
#[tokio::test]
async fn source_observation_handoff_drains_prior_root_event() {
    let root = TempDir::new().expect("source observation root");
    let cache = GitWorkspaceCache::new();
    let observation = cache
        .begin_source_path_change_observation_after_barrier(root.path())
    ...
    let handoff = cache
        .finish_source_path_change_observation_and_continue(&observation)
        .await
        .expect("finish source observation containing a root event");
    assert!(
        handoff.had_non_runtime_change,
        "a broad root event before the boundary must remain fail-closed evidence"
    );
    assert!(
        handoff.exact_paths.is_empty(),
        "a broad root event is deliberately not exact source-path evidence"
    );

    let continuation = cache
        .finish_source_path_change_observation_and_continue(&handoff.continuation)
        .await
        .expect("finish continuation after the prior root event");
    assert!(
        !continuation.had_non_runtime_change,
        "the root event before the handoff must be drained into the prior interval"
    );
    assert!(
        continuation.exact_paths.is_empty(),
        "the root event before the handoff must not leak into continuation evidence"
    );
}
