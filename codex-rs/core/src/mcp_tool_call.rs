...
async fn execute_approved_mcp_tool_call(
    ...
    .await;
    let raw_server_result = execution
        .as_ref()
        .ok()
        .map(|execution| execution.raw_server_result.clone());
