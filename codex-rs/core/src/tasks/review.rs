...
impl SessionTask for ReviewTask {
    ...
    async fn run(
        ...
        let (output, structured) = match start_review_conversation(
            session.clone(),
            ctx.clone(),
            user_input,
            cancellation_token.clone(),
...
/// Otherwise, attempt to extract the first JSON object substring for display,
/// but mark it unstructured so surrounding prose or fences can never authorize
...
mod tests {
    ...
    #[test]
    fn unstructured_review_text_cannot_be_a_clean_receipt() {
        let (output, structured) = parse_review_output_event("looks clean");
        assert!(!structured);
        assert_eq!(output.overall_explanation, "looks clean");
    }

    #[test]
    fn structured_review_output_is_accepted_as_a_receipt() {
        let expected = ReviewOutputEvent::default();
        let encoded = serde_json::to_string(&expected).expect("review output");
        let (actual, structured) = parse_review_output_event(&encoded);
        assert!(structured);
        assert_eq!(actual.findings, expected.findings);
        assert_eq!(actual.overall_explanation, expected.overall_explanation);
    }

    #[test]
    fn fenced_structured_review_output_cannot_authorize_a_clean_receipt() {
        let expected = ReviewOutputEvent::default();
        let encoded = serde_json::to_string(&expected).expect("review output");
        let (actual, structured) = parse_review_output_event(&format!("```json\n{encoded}\n```"));
        assert!(!structured);
        assert_eq!(actual.findings, expected.findings);
        assert_eq!(actual.overall_explanation, expected.overall_explanation);
    }
