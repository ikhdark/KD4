...
async fn logical_artifact_for_test(
    codex_home: &Path,
    text: &str,
) -> (LogicalArtifactMetadata, Vec<u8>) {
    let artifact =
        create_canonical_output_artifact(codex_home, "thread", &CanonicalToolResult::text(text))
            .await;
    ...
    let path = codex_home
        ...
        .join("thread")
        .join(format!("{id}.log"));
    let metadata = load_logical_metadata(
        &path,
        id.parse::<ToolOutputArtifactId>().expect("artifact id"),
    )
...
///
/// Normalization exists to remove redundant work, so it may still merge when
/// the merged range can be returned whole.
#[tokio::test(flavor = "current_thread")]
#[serial_test::serial(command_output_artifact)]
async fn adjacent_fitting_selectors_are_not_merged_into_one_that_no_longer_fits() {
    let temp = tempfile::tempdir().expect("tempdir");
    // One 60-line block fits the fragment ceiling; twelve of them together are
    // past what a single result can return, even after internal draining.
    ...
    let selectors = (0..12)
        .map(|block| ToolOutputSelector::Lines {
            start: block * 60 + 1,
            end: block * 60 + 60,
        })
        .collect::<Vec<_>>();
    ...
    let covered_lines = normalized
        ...
        .map(|selector| match selector {
            ToolOutputSelector::Lines { start, end } => end - start + 1,
            other => panic!("line selectors normalize to line selectors: {other:?}"),
        })
        .sum::<usize>();
    assert_eq!(
        covered_lines,
        selectors.len() * 60,
        "normalization must preserve the requested coverage"
    );
}
...
/// to read a large range in hundreds of tiny chunks, one round trip each.
#[tokio::test(flavor = "current_thread")]
#[serial_test::serial(command_output_artifact)]
async fn an_oversize_range_advertises_a_continuation_sized_for_its_real_bytes() {
    let temp = tempfile::tempdir().expect("tempdir");
    // Ordinary source text: nothing escapes, so a synthetic probe and the real
    // bytes disagree by the full escape factor.
    let text = (0..2_000)
        .map(|index| format!("pub fn handler_{index}(input: &str) -> String {{ input.into() }}"))
        .collect::<Vec<_>>()
        .join("\n");
    let artifact = create_canonical_output_artifact(
        temp.path(),
        "thread",
        &CanonicalToolResult::text(text.clone()),
    )
    .await;
    ...
    .await
    .expect("read oversize range");

    let result = &recovered.results[0];
    assert_eq!(result.status, ToolOutputSelectorStatus::SelectorTooLarge);
    let continuation = result
        .continuation
        .clone()
        .expect("an oversize range advertises how to continue");
    let ToolOutputSelector::Bytes { start, end } = continuation else {
        panic!("byte ranges continue by bytes: {continuation:?}");
    };
    assert_eq!(start, 0);
    let advertised = end - start;
    // The fragment ceiling is 1,000 tokens; plain text costs about four bytes
    // per token, so a correctly sized continuation is in the low thousands of
    // bytes. The zero-probe estimate advertised a few hundred.
    assert!(
        advertised > 1_500,
        "a continuation sized on real bytes must carry a useful chunk, got {advertised} bytes"
    );

    // And it must actually fit: reading exactly what was advertised succeeds.
    let followed = read_tool_output_selectors(
        temp.path(),
        "thread",
        &artifact_id,
        vec![ToolOutputSelector::Bytes { start, end }],
    )
    .await
    .expect("follow the advertised continuation");
    assert_eq!(
        followed.results[0].status,
        ToolOutputSelectorStatus::Ok,
        "the advertised continuation must be readable in one call"
    );
}

/// A uniform plan promises every chunk it names can be drained. A range the
/// auto-drain path would refuse must not advertise one.
#[tokio::test(flavor = "current_thread")]
#[serial_test::serial(command_output_artifact)]
async fn a_range_the_auto_drain_path_refuses_advertises_no_subdivision_plan() {
    let temp = tempfile::tempdir().expect("tempdir");
    // Well past MAX_AUTOMATIC_SUBDIVISION_BYTES, so no uniform plan can hold.
    let text = "abcdefghij".repeat(30_000);
    let artifact = create_canonical_output_artifact(
        temp.path(),
        "thread",
        &CanonicalToolResult::text(text.clone()),
    ...
    assert_eq!(result.status, ToolOutputSelectorStatus::SelectorTooLarge);
    assert!(
        result.subdivision_plan.is_none(),
        "a plan outside the auto-drain limits must not be advertised: {:?}",
        result.subdivision_plan
    );
    assert!(
        result.continuation.is_some(),
        "the recomputed continuation is still the way forward"
    );
}

/// Preservation: a range the auto-drain path accepts still gets its plan, and
/// still drains without extra round trips.
#[tokio::test(flavor = "current_thread")]
#[serial_test::serial(command_output_artifact)]
async fn an_auto_drain_eligible_range_still_completes_in_one_call() {
    let temp = tempfile::tempdir().expect("tempdir");
    // Inside MAX_AUTOMATIC_SUBDIVISION_BYTES and the final ceiling.
    let text = "abcdefghij".repeat(1_500);
    let artifact = create_canonical_output_artifact(
        temp.path(),
        "thread",
        &CanonicalToolResult::text(text.clone()),
    ...
    .await
    .expect("read in-limit range");

    assert_eq!(
        recovered.results[0].status,
        ToolOutputSelectorStatus::Ok,
        "an in-limit range auto-drains rather than reporting too large"
    );
    assert_eq!(recovered.results[0].text.as_deref(), Some(text.as_str()));
}

/// Preservation: merging still removes genuinely redundant work when the merged
/// range fits, so a caller that asks for adjacent slivers gets one clean range.
#[tokio::test(flavor = "current_thread")]
#[serial_test::serial(command_output_artifact)]
async fn small_adjacent_selectors_still_normalize_into_one_range() {
    let temp = tempfile::tempdir().expect("tempdir");
    let text = (0..40)
        ...
        .join("\n");
    let artifact =
        create_canonical_output_artifact(temp.path(), "thread", &CanonicalToolResult::text(text))
            .await;
    let artifact_id = artifact
        .artifact_id()
        .expect("canonical artifact ID")
    ...
    let recovered = read_tool_output_selectors(
        ...
        &artifact_id,
        vec![
            ToolOutputSelector::Lines { start: 1, end: 10 },
            ToolOutputSelector::Lines { start: 11, end: 20 },
        ],
    )
    .await
    .expect("read selected lines");

    assert_eq!(
        recovered.results.len(),
        1,
        "adjacent ranges that fit are still normalized into one"
    );
    assert_eq!(
        recovered.results[0].selector,
        ToolOutputSelector::Lines { start: 1, end: 20 }
    );
...
#[tokio::test]
#[serial_test::serial(command_output_artifact)]
async fn token_efficiency_reduced_output_notice_delivers_its_exact_bounded_selection() {
    let temp = tempfile::tempdir().expect("tempdir");
    for body in [b"retained output".to_vec(), vec![b'x'; 2048]] {
        ...
        assert!(notice.contains("Selection completeness does not mean full artifact delivery"));
        let start = notice
            .find("{\"artifact_id\"")
        ...
        let output = read_tool_output_selectors(
            ...
            serde_json::from_value(args["selectors"].clone()).expect("selectors"),
        )
        .await
        .expect("advertised selection must be readable");
...
async fn oversized_json_pointer_exposes_exact_chunkable_canonical_range() {
    ...
    // This range is past the auto-drain limits, so no uniform plan can promise
    // that every chunk it names is drainable. Recovery follows the advertised
    // continuation, recomputed against the real bytes on each read.
    assert!(
        selected.subdivision_plan.is_none(),
        "a range the auto-drain path refuses must not advertise a uniform plan: {:?}",
        selected.subdivision_plan
    );
    ...
    let ToolOutputSelector::Bytes { start, .. } = continuation else {
        panic!("byte ranges continue by bytes: {continuation:?}");
    };
    assert_eq!(*start, expected_range.start);

    let mut recovered = Vec::new();
    let mut next = Some(continuation.clone());
    let mut reads = 0_u32;
    while let Some(ToolOutputSelector::Bytes { start, end }) = next {
        reads += 1;
        assert!(
            reads < 512,
            "continuations must advance, not loop: recovered {} of {} bytes",
            recovered.len(),
            expected.len()
        );
        assert!(end > start, "each continuation must advance past its start");
        let chunk = read_tool_output_selectors(
            temp.path(),
            "thread",
            &artifact_id,
            vec![ToolOutputSelector::Bytes { start, end }],
        ...
        assert_eq!(
            chunk.status,
            ToolOutputSelectorStatus::Ok,
            "an advertised continuation must be readable in one call"
        );
        ...
        next = (end < expected_range.end).then_some({
            // The next read asks for the remainder and is told how much of it
            // fits, so each step is sized against the bytes it will return.
            ToolOutputSelector::Bytes {
                start: end,
                end: expected_range.end,
            }
        });
        if let Some(ToolOutputSelector::Bytes { start, end }) = next.clone() {
            let probe = read_tool_output_selectors(
                temp.path(),
                "thread",
                &artifact_id,
                vec![ToolOutputSelector::Bytes { start, end }],
            ...
            .await
            .expect("probe the remainder");
            let probe = &probe.results[0];
            if probe.status == ToolOutputSelectorStatus::SelectorTooLarge {
                next = probe.continuation.clone();
            }
        }
    }
    assert_eq!(
        recovered, expected,
        "continuations must reconstruct the range without gaps or duplication"
    );
