...
pub fn create_update_plan_tool(include_task_evidence: bool) -> ToolSpec {
    if !include_task_evidence {
        return create_checklist_tool();
    }
...
fn create_checklist_tool() -> ToolSpec {
    let status = JsonSchema::string_enum(
        vec![
            json!("pending"),
            json!("in_progress"),
            json!("implemented"),
                ...
                (
                    ...
                    JsonSchema::array(
                        JsonSchema::string_enum(
                            ...
                            Some("Completion-review surface role.".to_string()),
                        ),
                        Some(
                            "Semantic roles affected by this step when paths do not fully identify the runtime contract."
                                .to_string(),
                        ),
                    ),
                ),
                (
                    "validation_asset_paths".to_string(),
                    JsonSchema::array(
                        JsonSchema::string(Some(
                            "Repository-relative validation asset path.".to_string(),
                        )),
                        Some(
                            "Validation fixtures, goldens, snapshots, or other oracle assets affected by this step."
                                .to_string(),
                        ),
                    ),
                ),
