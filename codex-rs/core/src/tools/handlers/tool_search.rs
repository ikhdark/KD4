...
fn index_output_name(
    ...
    if let Some(namespace) = &tool_name.namespace {
        // A qualified lookup is an identity request even when the caller
        // separates the namespace and member with whitespace. Without these
        // aliases, description-length scoring can prefer a sibling operation.
        let source = namespace.strip_prefix("mcp__").unwrap_or(namespace);
        for qualifier in [namespace.as_str(), source] {
            aliases.push(format!("{qualifier} {}", tool_name.name));
            aliases.push(format!("+{qualifier} {}", tool_name.name));
        }
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn qualified_search_activates_the_requested_member_in_one_lookup() {
        let reset = search_info(
            "script_host eval reset kernel",
            None,
            "script_host",
            "eval_reset",
        );
        let evaluate = search_info(
            &format!(
                "script_host eval {}",
                "execute persistent JavaScript with arguments ".repeat(80)
            ),
            None,
            "script_host",
            "eval",
        );
        let unrelated = search_info("script_host eval", None, "another_host", "eval");
        // Cover both one-entry-per-member inventories and namespace containers
        // whose first member is not the requested operation.
        for grouped in [false, true] {
            let mut inventory = vec![reset.clone(), evaluate.clone(), unrelated.clone()];
            if grouped {
                let LoadableToolSpec::Namespace(target) = &evaluate.entry.output else {
                    panic!("namespace")
                };
                let LoadableToolSpec::Namespace(container) = &mut inventory[0].entry.output else {
                    panic!("namespace")
                };
                container.tools.extend(target.tools.clone());
                inventory[0].entry.tool_names.push("eval".into());
                inventory.remove(1);
            }
            let handler = ToolSearchHandler::new(inventory);
            for query in [
                "+script_host eval",
                "mcp__script_host eval",
                "  SCRIPT_HOST   EVAL  ",
                "+script_host eval",
            ] {
                let (session, mut turn, _events) = make_session_and_context_with_rx().await;
                ...
                let wanted = ToolName::namespaced("mcp__script_host", "eval");
                turn.refresh_deferred_tool_capabilities(Arc::new(
                    [
                        (wanted.clone(), "v1".to_string()),
                        (
                            ToolName::namespaced("mcp__script_host", "eval_reset"),
                            "v1".to_string(),
                        ),
                        (
                            ToolName::namespaced("mcp__another_host", "eval"),
                            "v1".to_string(),
                        ),
                    ]
                    .into_iter()
                ...
                assert_eq!(tools[0]["name"], "mcp__script_host");
                assert_eq!(tools[0]["tools"].as_array().unwrap().len(), 1);
                assert_eq!(tools[0]["tools"][0]["name"], "eval");
                assert_eq!(
                    turn.activated_deferred_tools(),
                ...
                let schemas = history
                    ...
                    .expect(
                        "requested callable must be available without another discovery generation",
                    );
                let advertised: Vec<serde_json::Value> = serde_json::from_str(schemas).unwrap();
                assert_eq!(advertised.len(), 1);
                assert_eq!(advertised[0]["tools"][0]["name"], "eval");
            }
        }
    }

    #[test]
    fn qualified_name_aliases_preserve_descriptive_search_and_source_ambiguity() {
        let handler = ToolSearchHandler::new(vec![
            search_info("reset kernel", None, "script_host", "eval_reset"),
            search_info("execute code", None, "script_host", "eval"),
            search_info("execute code", None, "another_host", "eval"),
        ]);
        for query in ["reset kernel", "+script_host eval reset kernel"] {
            assert_eq!(
                handler.search(query, 1).unwrap().activation_tools,
                vec![ToolName::namespaced("mcp__script_host", "eval_reset")]
            );
        }
        assert_eq!(
            handler.search("eval", 2).unwrap().activation_tools,
            vec![
                ToolName::namespaced("mcp__another_host", "eval"),
                ToolName::namespaced("mcp__script_host", "eval")
            ]
        );
        assert!(
            handler
                .search("missing_source missing_member", 1)
                .unwrap()
                .tools
                .is_empty()
        );
