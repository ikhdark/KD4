...
fn exec_server_env_keeps_command_native_and_carries_sandbox_context() {
    ...
    assert!(!request.descendant_containment_established);
}

#[test]
fn managed_mutation_requires_a_strong_local_descendant_container() {
    let cwd: AbsolutePathBuf = std::env::current_dir()
        .expect("current dir")
        .try_into()
        .expect("absolute cwd");
    ...
    assert!(
        attempt
            .require_strong_descendant_containment(
                /*required*/ false, /*environment_is_remote*/ true,
                /*additional_permissions*/ None,
            )
            .is_ok(),
        "unmanaged execution must retain its existing behavior"
    );
    assert!(
        attempt
            .require_strong_descendant_containment(
                /*required*/ true, /*environment_is_remote*/ false,
                /*additional_permissions*/ None,
            )
            .is_err()
    );
    ...
    #[cfg(target_os = "linux")]
    {
        attempt.sandbox = SandboxType::LinuxSeccomp;
        assert!(attempt.has_strong_descendant_containment(None));
        attempt.use_legacy_landlock = true;
        assert!(!attempt.has_strong_descendant_containment(None));
        let unrestricted_permissions = PermissionProfile::unrestricted();
        attempt.permissions = &unrestricted_permissions;
        attempt.use_legacy_landlock = false;
        assert!(
            !attempt.has_strong_descendant_containment(None),
            "full-disk-write execution bypasses bubblewrap"
        );
        attempt.enforce_managed_network = true;
        assert!(
            attempt.has_strong_descendant_containment(None),
            "proxy-only networking forces bubblewrap even for full-disk-write execution"
        );
        attempt.permissions = &permissions;
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        assert!(attempt.has_strong_descendant_containment(None));
        attempt.windows_sandbox_level = WindowsSandboxLevel::RestrictedToken;
        assert!(!attempt.has_strong_descendant_containment(None));
    }
    ...
    #[cfg(any(target_os = "linux", target_os = "windows"))]
    {
        ...
        assert!(
            attempt
                .require_strong_descendant_containment(
                    /*required*/ true, /*environment_is_remote*/ true,
                    /*additional_permissions*/ None,
                )
                .is_err(),
            "remote process containment is not owned by the local runtime"
        );
    }
}
