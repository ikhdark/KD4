...
#[tokio::test]
async fn validation_lane_is_held_by_the_running_process_and_released_at_exit() -> anyhow::Result<()>
{
    let root = tempfile::tempdir()?;
    let repo = root.path().join("repo");
    std::fs::create_dir_all(&repo)?;
    ...
    std::fs::write(repo.join("source.txt"), "captured\n")?;
    let home = root.path().join("home");
    crate::workspace_transaction::begin(&home, "lane-test", &repo)?;
    let snapshot = crate::workspace_transaction::validation_snapshot(&home, "lane-test")?;
    let lease =
        || crate::workspace_transaction::lease_validation_lane(&home, "lane-test", &snapshot);
    let python = which::which("python").or_else(|_| which::which("python3"))?;
    let spawned = codex_utils_pty::spawn_pipe_process_no_stdin(
        &python.to_string_lossy(),
    ...
    .await?;
    process.hold_until_exit(lease()?.expect("idle lane"));
    let others = (0..3)
        .map(|_| lease().map(|lane| lane.expect("overflow lane")))
        .collect::<anyhow::Result<Vec<_>>>()?;
    assert!(
        lease()?.is_none(),
        "the running process must keep its lane leased"
    );
    drop(others);
    tokio::time::timeout(
        Duration::from_secs(10),
    ...
    .await?;
    // The exited process stays registered; its lane must be free regardless.
