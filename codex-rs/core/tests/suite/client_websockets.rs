...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[traced_test]
async fn responses_websocket_timing_uses_per_turn_id_on_reused_connection() {
    skip_if_no_network!();
    ...
    let prompt_one = prompt_with_input(vec![message_item("hello")]);
    let prompt_two = prompt_with_input(vec![message_item("again")]);
    let turn_one_metadata = turn_metadata(&harness, Some("turn-1"));
    let turn_two_metadata = turn_metadata(&harness, Some("turn-2"));
        ...
        let events = lines
            ...
            .filter(|line| {
                line.contains("codex_otel.log_only")
                    && line.contains("codex.responses_websocket_timing")
            })
            .copied()
            .collect::<Vec<_>>();
        if events.len() != 2 {
            return Err(format!(
                "expected exactly two provider timing events, got {events:?}"
            ));
        }
        if !events
            .iter()
            .any(|line| line.contains("turn.id=\"turn-1\""))
        {
            return Err(format!("missing turn-1 provider timing event: {events:?}"));
        }
        if !events
            .iter()
            .any(|line| line.contains("turn.id=\"turn-2\""))
        {
            return Err(format!("missing turn-2 provider timing event: {events:?}"));
        }
        Ok(())
    });
