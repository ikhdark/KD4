...
async fn assert_exec_process_streams_output(use_remote: bool) -> Result<()> {
    ...
    let session = context
        ...
        .start(ExecParams {
            ...
            argv: powershell(
                "Start-Sleep -Milliseconds 50; [Console]::Out.Write(\"session output`n\")",
            ),
            cwd: PathUri::from_host_native_path(std::env::current_dir()?)?,
            env_policy: /*env_policy*/ None,
            env: Default::default(),
...
async fn assert_exec_process_pushes_events(use_remote: bool) -> Result<()> {
    ...
    let session = context
        ...
        .start(ExecParams {
            process_id: process_id.clone().into(),
            argv: powershell(
                "[Console]::Out.Write(\"event output`n\"); Start-Sleep -Milliseconds 100; [Console]::Error.Write(\"event err`n\"); Start-Sleep -Milliseconds 100; exit 7",
            ),
            cwd: PathUri::from_host_native_path(std::env::current_dir()?)?,
            env_policy: /*env_policy*/ None,
            env: Default::default(),
...
async fn assert_exec_process_replays_events_after_close(use_remote: bool) -> Result<()> {
    ...
    let session = context
        ...
        .start(ExecParams {
            process_id: process_id.clone().into(),
            argv: powershell(
                "[Console]::Out.Write(\"late one`n\"); [Console]::Out.Write(\"late two`n\")",
            ),
            cwd: PathUri::from_host_native_path(std::env::current_dir()?)?,
            env_policy: /*env_policy*/ None,
            env: Default::default(),
...
async fn assert_exec_process_write_then_read_without_tty(use_remote: bool) -> Result<()> {
    ...
    let session = context
        ...
        .start(ExecParams {
            process_id: process_id.clone().into(),
            argv: powershell(
                "$line = [Console]::In.ReadLine(); [Console]::Out.Write(\"from-stdin:$line`n\")",
            ),
            cwd: PathUri::from_host_native_path(std::env::current_dir()?)?,
            env_policy: /*env_policy*/ None,
            env: Default::default(),
...
async fn assert_exec_process_rejects_write_without_pipe_stdin(use_remote: bool) -> Result<()> {
    ...
    let session = context
        ...
        .start(ExecParams {
            process_id: process_id.clone().into(),
            argv: powershell(
                "Start-Sleep -Milliseconds 300; $line = [Console]::In.ReadLine(); if ($null -eq $line) { [Console]::Out.Write(\"eof`n\") } else { [Console]::Out.Write(\"read:$line`n\") }",
            ),
            cwd: PathUri::from_host_native_path(std::env::current_dir()?)?,
            env_policy: /*env_policy*/ None,
            env: Default::default(),
