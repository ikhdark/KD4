...
use crate::state::EmittedTurnWarnings;
    ...
    fn contribute_thread_context<'a>(
        ...
        Box::pin(async move {
            ...
            for warning in &catalog.warnings {
                self.emit_warning(thread_store, thread_store.level_id(), warning.clone());
            }
            available_skills_fragment(&catalog)
                .map(|fragment| PromptFragment::developer_capability(fragment.render()))
                .into_iter()
    ...
    C: Send + Sync + 'static,
{
    fn contribute<'a>(
        ...
        Box::pin(async move {
            ...
            for warning in &catalog.warnings {
                self.emit_turn_warning(thread_store, turn_store, &input.turn_id, warning.clone());
            }
                    ...
                    .await
                {
                    Ok(read_result) => {
                        let Some((fragment, truncated)) =
                            ...
                            self.emit_turn_warning(
                                ...
                                turn_store,
                                &input.turn_id,
                                format!("Skill `{}` could not fit its complete recovery identity in the selected-instruction budget.", entry.name),
                            );
                    ...
                    Err(message) => {
                        self.emit_turn_warning(
                            ...
                            turn_store,
                            &input.turn_id,
                            format!("Failed to load skill `{}`: {message}", entry.name),
                        );
...
impl<C> SkillsExtension<C> {
    ...
    async fn list_skills(
        ...
        if include_orchestrator_skills {
            ...
            let orchestrator_catalog = if estimate {
                thread_state
                    .estimate_orchestrator_catalog_snapshot(mcp_resources.as_deref(), initialize)
                    .await
            } else {
                thread_state
                    .orchestrator_catalog_snapshot(mcp_resources.as_deref(), initialize)
                    .await
            };
    ...
    fn emit_turn_warning(
        ...
        if turn_store
            .get_or_init(EmittedTurnWarnings::default)
            .first_emission(&message)
        {
            self.emit_warning(thread_store, turn_id, message);
        }
    }
    ...
    fn emit_warning(&self, thread_store: &ExtensionData, turn_id: &str, message: String) {
        ...
        match ThreadId::from_string(thread_store.level_id()) {
            Ok(thread_id) => self.event_sink.emit_for_thread(thread_id, event),
            Err(_) => self.event_sink.emit(event),
        }
    }
}
