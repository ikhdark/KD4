...
async fn write_generated_file_atomic(path: &Path, contents: &str) -> io::Result<()> {
    ...
    tokio::task::spawn_blocking(move || {
        write_generated_file_atomic_sync(&path, contents.as_bytes())
    })
    .await
...
mod tests {
    ...
    async fn write_generated_file_writes_atomically_and_removes_temp_file() {
        ...
        write_generated_file(&path, "hello")
            .await
            .expect("write file");
    ...
    #[tokio::test]
    async fn write_generated_file_skips_unchanged_read_only_file() {
        let tempdir = tempfile::tempdir().expect("tempdir");
        let path = tempdir.path().join("generated.md");
        write_generated_file(&path, "unchanged")
            .await
        ...
        write_generated_file(&path, "unchanged")
            .await
            .expect("unchanged write should return before replacement");
