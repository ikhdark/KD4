...
pub struct ModelProviderInfo {
    ...
    /// Compatibility field retained for crates mirrored from upstream.
    ///
    /// This fork does not route standalone web search through custom model
    /// providers, so the field is intentionally excluded from configuration.
    #[doc(hidden)]
    #[serde(skip)]
    #[schemars(skip)]
