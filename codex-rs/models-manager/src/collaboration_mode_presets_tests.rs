...
fn default_mode_instructions_keep_only_the_compact_execution_contract() {
    ...
    assert!(normalized_instructions.contains("meaningful options allowed by its schema"));
...
fn plan_mode_instructions_preserve_planning_contract() {
    ...
    assert!(normalized_instructions.contains("Use fresh evidence already in context"));
