...
#[tokio::test]
async fn sol_6_bundled_catalog_resolves_picker_and_runtime_metadata() {
    ...
    let sol = models
        ...
        .expect("Sol 6 must be available without a remote catalog");
    assert!(sol.show_in_picker);
    ...
    assert_eq!(sol.default_service_tier.as_deref(), Some("priority"));

    for slug in ["gpt-6-sol", "gpt-6-sol-preview", "openai/gpt-6-sol"] {
        let model = manager
            .get_model_info(slug, &ModelsManagerConfig::default())
            .await;
