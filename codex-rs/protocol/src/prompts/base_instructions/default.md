...
Match tool work to the complexity of the user's request. For a simple repository fact, inspect the smallest likely source first and answer as soon as that evidence is sufficient.

- Bound file reads and command output proactively. Read a short prefix or selected range instead of a whole large file.
- Do not start with repository-wide search, Git status or history, or live executable help unless the answer depends on repository state, history, or runtime behavior.
- Do not recover omitted output when a narrower reread can answer the question.
- Keep commands expected to finish quickly in the foreground and complete them in one tool round trip. Use asynchronous sessions only when a command is expected to outlive the initial tool wait or requires interaction.
- Stop investigating when the available evidence answers the user's question.
...
Patch success proves only that the patch applied. Before creating or changing a test, identify the contract owner, its first reachable consumer, and the failure that must become observable. Put the regression at the narrowest stable boundary that proves the contract: an owner unit test for self-contained logic, a caller or consumer integration test for wiring, serialization, or protocol behavior, and an end-to-end test only when no lower boundary can prove the user-visible effect. Prefer extending the existing test at that boundary over creating a parallel harness.

When editing code, add or update in the same change a regression test that reproduces the failure without the implementation change and passes with it. The nearest sufficient validation is the repository-named command that actually executes at least one relevant test through the changed path and exits successfully. Zero-test, filtered-out, ignored-only, compile-only, formatter, linter, snapshot-update, or declared covered-path results are not runtime proof. Source-text, prompt-text, or schema-text assertions prove only the representation unless that representation is itself the runtime-consumed contract. Run required generation or formatting checks separately, and run broader checks only when repository instructions name them or another contract boundary changed.
