...
async fn project_updates_preserve_omitted_fields_across_concurrent_writers() -> anyhow::Result<()> {
    let home = unique_temp_dir();
    let runtime_one = StateRuntime::init(home.clone(), "test-provider".to_string()).await?;
    let runtime_two = StateRuntime::init(home.clone(), "test-provider".to_string()).await?;
...
#[tokio::test]
async fn explicit_project_update_is_atomic_and_survives_reconciliation() -> anyhow::Result<()> {
    let home = unique_temp_dir();
    let runtime = StateRuntime::init(home.clone(), "test-provider".to_string()).await?;
    let project = runtime
        .create_project(
    ...
    assert_eq!(runtime.get_thread(thread_id).await?.unwrap(), before);
    // A rollout-derived snapshot cannot erase host-owned membership.
    metadata.title = "Reconciled".to_string();
