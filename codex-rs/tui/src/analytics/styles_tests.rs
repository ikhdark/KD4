        ...
        with_test_default_colors(colors, || {
            for kind in [
                ...
                for section in view.visible_sections() {
                    ...
                    let heading = &tab[0];
                    let active = accent_style().reversed();
                    assert_eq!(heading.fg, active.fg.unwrap());
                    assert_eq!(heading.bg, Color::Reset);
                    assert!(heading.modifier.contains(Modifier::REVERSED));
                    ...
                    if theme == "light" {
                            ...
                            .filter(|(_, cell)| !cell.symbol().trim().is_empty())
                        {
                            ...
                            if cell.fg == Color::White && cell.bg == Color::Reset {
        ...
        || {
            for kind in [models::AccountKind::Business, models::AccountKind::Consumer] {
                ...
                for width in [40, 120] {
                    ...
                    let mut buffer = Buffer::empty(area);
                    view.render(area, &mut buffer);
                    let selection = super::styles::selection_style().bg.unwrap();
