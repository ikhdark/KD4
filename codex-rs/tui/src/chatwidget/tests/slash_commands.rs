...
async fn queued_empty_bang_shell_reports_help_when_dequeued_and_drains_next_input() {
    ...
    assert!(
        rendered.contains("Example: !Get-ChildItem"),
        "expected Windows shell example, got {rendered:?}"
    );
...
async fn removed_memory_command_is_rejected_without_submission() {
    ...
    let rendered = cells
        ...
        .join("\n");
    assert!(
        rendered.contains("Unrecognized command '/debug-m-drop'"),
        "expected unrecognized-command message, got: {rendered:?}"
    );
    assert_eq!(chat.bottom_pane.composer_text(), "/debug-m-drop");
    assert_eq!(recall_latest_after_clearing(&mut chat), "");
