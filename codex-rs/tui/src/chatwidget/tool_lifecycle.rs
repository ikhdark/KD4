...
impl ChatWidget {
    pub(super) fn on_patch_apply_updated(
        &mut self,
        item_id: String,
        ...
        }) && cell.is_streaming_item(&item_id)
        {
            cell.update_changes(changes);
        ...
        let mut cell = history_cell::new_streaming_patch_event(item_id, &self.config.cwd);
        cell.update_changes(changes);
    ...
    pub(super) fn on_patch_apply_begin(
        ...
        self.record_visible_turn_activity();
        let replaces_streaming_cell = self
            .transcript
            .active_cell
            .as_ref()
            .and_then(|cell| {
                cell.as_any()
                    .downcast_ref::<history_cell::PatchHistoryCell>()
            })
            .is_some_and(|cell| cell.is_streaming_item(&item_id));
        if replaces_streaming_cell {
    ...
    pub(super) fn on_file_change_completed(&mut self, item: ThreadItem) {
        self.defer_or_handle_owned(
            ...
            super::ChatWidget::handle_file_change_completed_now,
        );
    }
    ...
    pub(super) fn on_mcp_tool_call_started(&mut self, item: ThreadItem) {
        self.defer_or_handle_owned(
            ...
            super::ChatWidget::handle_mcp_tool_call_started_now,
        );
    }
    ...
    pub(super) fn on_mcp_tool_call_completed(&mut self, item: ThreadItem) {
        self.defer_or_handle_owned(
            item,
            super::interrupts::InterruptManager::push_item_completed,
            super::ChatWidget::handle_mcp_tool_call_completed_now,
        );
    }
