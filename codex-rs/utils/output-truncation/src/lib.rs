...
pub fn resolve_output_limits(
    ...
    let default_limit = match classify_output(outcome, command_text, output_text) {
        OutputBudgetClass::Success => DEFAULT_SUCCESS_OUTPUT_TOKENS,
        OutputBudgetClass::FailureOrTimeout => DEFAULT_FAILURE_OUTPUT_TOKENS,
        OutputBudgetClass::Diagnostic => DEFAULT_DIAGNOSTIC_OUTPUT_TOKENS,
    };
    let applied_limit = requested_limit.unwrap_or(default_limit).min(hard_limit);
...
fn classify_output(
    ...
    if is_high_signal_diagnostic(command_text, output_text) {
        OutputBudgetClass::Diagnostic
    } else {
        match outcome {
            OutputOutcome::Success => OutputBudgetClass::Success,
            OutputOutcome::Failure | OutputOutcome::TimedOut => OutputBudgetClass::FailureOrTimeout,
        }
    }
}
