...
class RolloutSnapshotTest(unittest.TestCase):
    def test_compressed_rollout_cli_accepts_path_directory_and_uuid(self):
        ...
        rows = [
            {"timestamp": "2026-08-17T00:00:00Z", "type": "event_msg",
             "payload": {"type": "task_started", "turn_id": "turn-1"}},
            {"timestamp": "2026-08-17T00:00:01Z", "type": "event_msg",
             "payload": {"type": "task_complete", "turn_id": "turn-1", "timing": {
                 "schemaVersion": 25, "milestones": {
                     "firstDomainActionMs": 12.5, "firstUsefulActionMs": 12.5}}}},
        ]
        data = ("\n".join(json.dumps(row) for row in rows) + "\n").encode()
        # Rust's streaming encoder can emit frames without a content size.
        compressed = zstd.compress(data, options={zstd.CompressionParameter.content_size_flag: 0})
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            plain = root / f"rollout-{session_id}.jsonl"
            path = plain.with_name(plain.name + ".zst")
            path.write_bytes(compressed)
            metadata = {"path": str(path.resolve()), "byteLength": len(compressed),
                        "sha256": hashlib.sha256(compressed).hexdigest()}
            for source in (str(path), str(plain), str(root), session_id):
                with self.subTest(source=source), contextlib.redirect_stdout(io.StringIO()) as stdout:
                    self.assertEqual(kd4_turn_latency_audit.main([
                        source, "--sessions-root", str(root), "--repo-root", str(root), "--json",
                    ]), 0)
                ...
                self.assertEqual(action["canonical"]["startToFirstDomainActionMs"]["p50"], 12.5)
            ...
            self.assertEqual(standalone["canonical"]["startToFirstDomainActionMs"]["p50"], 12.5)
            output = root / "captured.jsonl.zst"
            with contextlib.redirect_stdout(io.StringIO()):
                self.assertEqual(rollout_snapshot.main([str(path), "--output", str(output)]), 0)
    ...
    def test_plain_rollout_wins_over_compressed_sibling_without_double_counting(self):
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            plain.with_name(plain.name + ".zst").write_bytes(b"stale compressed sibling")
            report = kd4_turn_latency_audit.analyze_session_path(root, root)
            self.assertEqual(report["coverage"]["snapshots"], [rollout_snapshot.read_rollout_snapshot(plain).metadata()])
            self.assertEqual(report["coverage"]["parseErrorCount"], 0)
            self.assertEqual(kd4_turn_latency_audit.resolve_rollout_source(session_id, root), plain.resolve())
