...
#[test]
fn ui() {
    let selected = std::env::var_os("KD4_ARGUMENT_COMMENT_LINT_UI_CASE")
        .map(|case| SelectedUiCase::new(&case))
        .transpose()
        .unwrap_or_else(|err| panic!("{err}"));
    let src_base = selected
        .as_ref()
        .map_or_else(|| std::path::Path::new("ui"), |selected| selected.path());

    dylint_testing::ui_test(env!("CARGO_PKG_NAME"), src_base);
}

#[cfg(test)]
struct SelectedUiCase {
    root: std::path::PathBuf,
}

#[cfg(test)]
impl SelectedUiCase {
    fn new(case: &std::ffi::OsStr) -> Result<Self, String> {
        let case_path = std::path::Path::new(case);
        let Some(case_name) = case_path.to_str() else {
            return Err("selected Dylint UI case is not valid UTF-8".to_string());
        };
        if case_path.components().count() != 1
            || case_name.is_empty()
            || !case_name
                .chars()
                .all(|ch| ch == '_' || ch.is_ascii_alphanumeric())
        {
            return Err(format!("invalid Dylint UI case `{case_name}`"));
        }

        let ui_dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("ui");
        let source = ui_dir.join(format!("{case_name}.rs"));
        if !source.is_file() {
            return Err(format!("unknown Dylint UI case `{case_name}`"));
        }

        let root = std::env::temp_dir().join(format!(
            "argument-comment-lint-ui-{}-{case_name}",
            std::process::id()
        ));
        if root.exists() {
            std::fs::remove_dir_all(&root).map_err(|err| {
                format!(
                    "failed to remove stale selected UI directory {}: {err}",
                    root.display()
                )
            })?;
        }
        std::fs::create_dir(&root).map_err(|err| {
            format!(
                "failed to create selected UI directory {}: {err}",
                root.display()
            )
        })?;

        for extension in ["rs", "stderr", "stdout", "fixed"] {
            let from = ui_dir.join(format!("{case_name}.{extension}"));
            if from.is_file() {
                let to = root.join(format!("{case_name}.{extension}"));
                std::fs::copy(&from, &to).map_err(|err| {
                    format!(
                        "failed to copy selected UI fixture {} to {}: {err}",
                        from.display(),
                        to.display()
                    )
                })?;
